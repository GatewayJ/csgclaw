...
注意：MCP command 在目标 runtime 环境内执行，filesystem server 的目录参数也必须是该 runtime 可见路径。MCP 只通过顶层 `mcp_config` 字段配置，或通过 `POST /api/v1/agents/{id}/mcp-servers` 按名称把 MCP catalog server 合并进 agent MCP 配置；`runtime_options.mcp` 不是受支持的输入。
...
- `POST /api/v1/agents/{id}/mcp-servers` 接收 `{ "names": [...] }`，把匹配的 MCP catalog server 配置合并进 agent MCP 配置
