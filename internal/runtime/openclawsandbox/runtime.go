...
func (r *Runtime) Provision(_ context.Context, req agentruntime.ProvisionRequest) error {
	...
	if _, err := EnsureConfigWithRuntimeOptions(agentHome, participantID, req.AgentID, gateway.Server, configModelFromProfile(profile), req.RuntimeOptions, fixedBaseURL(gateway.ManagerBaseURL), r.CurrentFeishuProvider()); err != nil {
		return err
	}
