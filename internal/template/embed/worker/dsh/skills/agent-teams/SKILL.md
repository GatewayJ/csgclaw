...
Do not use legacy `--bot-id` for team commands. If `csgclaw-cli team task claim --help` does not show `--participant-id`, the runtime has a stale `csgclaw-cli`; report that blocker and stop instead of retrying with `--bot-id`.
...
- For any team task that creates, inspects, or updates a project, use `~/.csgclaw/projects/{name}` as the shared project directory. Check there before creating a new project path.
- Keep project artifacts, notes, and generated files under the same `~/.csgclaw/projects/{name}` tree so managers and workers can inspect the same content.
